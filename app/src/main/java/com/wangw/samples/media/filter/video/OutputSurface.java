...
class OutputSurface implements SurfaceTexture.OnFrameAvailableListener {
    private static final String TAG = "OutputSurface_zl";
     ...
     */
    public OutputSurface(int width, int height, Context context, FilterManager.FilterType filterType) {
        if (width <= 0 || height <= 0) {
     ...
     */
    public OutputSurface(Context context, FilterManager.FilterType filterType) {
        setup(context, filterType);
    }
     ...
     */
    private void setup(Context context, FilterManager.FilterType filterType) {
...
//        mSurfaceTexture = new SurfaceTexture(mTextureRender.getTextureId());
        ...
        mFullFrameRect = new FullFrameRect(FilterManager.getCameraFilter(mCurrentFilterType, mContext));
        Matrix.setIdentityM(mSTMatrix, 0);

        mTextureId = mFullFrameRect.createTexture();
    ...
    public void changeFilter(FilterManager.FilterType filterType) {
        if (mFullFrameRect != null && filterType != mCurrentFilterType) {
            mFullFrameRect.changeProgram(FilterManager.getCameraFilter(filterType, mContext));
            mCurrentFilterType = filterType;
        }
    }
        ...
        synchronized (mFrameSyncObject) {
            while (!mFrameAvailable) {
                try {
                    ...
                    if (!mFrameAvailable) {
                        n++;
                        Log.i(TAG,"Surface frame wait,time: " + n);
...
//        mTextureRender.drawFrame(mSurfaceTexture);
        ...
        mFullFrameRect.getFilter().setTextureSize(720, 1280);
