...
public class CameraFilterSkinWhite extends CameraFilter {

    protected int mSkinWhiteTextureId;
    private int mTexelHeightUniformLocation;
    private int mTexelWidthUniformLocation;
    private int mToneCurveTextureUniformLocation;

    public CameraFilterSkinWhite(Context context) {
        super(context);
        mSkinWhiteTextureId = GlUtil.createTexture(GLES20.GL_TEXTURE_2D);
        initByteBuffer();
    }

    public CameraFilterSkinWhite(Context context, boolean isForCamera) {
    ...
    protected int createProgram(Context applicationContext) {
        return GlUtil.createProgram(applicationContext, R.raw.vertex_shader_default,
                R.raw.fragment_shader_ext_skinwhiten, mIsForCamera);
    }
    ...
    protected void getGLSLValues() {
        ...
        mTexelWidthUniformLocation = GLES20.glGetUniformLocation(mProgramHandle, "texelWidthOffset");
        mTexelHeightUniformLocation = GLES20.glGetUniformLocation(mProgramHandle, "texelHeightOffset");
    }

    @Override
    protected void bindTexture(int textureId) {
        super.bindTexture(textureId);
        if (mSkinWhiteTextureId != -1) {
            GLES20.glActiveTexture(GLES20.GL_TEXTURE3);
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mSkinWhiteTextureId);
    ...
    protected void bindGLSLValues(float[] mvpMatrix, FloatBuffer vertexBuffer, int coordsPerVertex,
        ...
        if (mIncomingWidth != 0 && mIncomingHeight != 0) {
            GLES20.glUniform1f(mTexelWidthUniformLocation, (1.0f / (float) mIncomingWidth));
            GLES20.glUniform1f(mTexelHeightUniformLocation, (1.0f / (float) mIncomingWidth));
        }
...
//        GLES20.glDisable(mToneCurveTextureUniformLocation);
//        GLES20.glDisable(mTexelWidthUniformLocation);
//        GLES20.glDisable(mTexelHeightUniformLocation);
    }
