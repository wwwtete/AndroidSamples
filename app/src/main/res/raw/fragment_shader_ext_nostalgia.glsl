...
{ 
	...
	vec4 textureColor = texture2D( uTexture, vec2(xCoordinate, yCoordinate)); 
	...
	highp vec2 centervTextureCoord = vec2(xCoordinate, yCoordinate); 
	highp vec2 oneStepLeftvTextureCoord = vec2(xCoordinate, yCoordinate) - firstOffset; 
	highp vec2 twoStepsLeftvTextureCoord = vec2(xCoordinate, yCoordinate) - secondOffset; 
	highp vec2 oneStepRightvTextureCoord = vec2(xCoordinate, yCoordinate) + firstOffset; 
    highp vec2 twoStepsRightvTextureCoord = vec2(xCoordinate, yCoordinate) + secondOffset; 

	lowp vec4 fragmentColor = texture2D(uTexture, vec2(centervTextureCoord.x, centervTextureCoord.y)) * 0.2270270270; 
	fragmentColor += texture2D(uTexture, vec2(oneStepLeftvTextureCoord.x, oneStepLeftvTextureCoord.y)) * 0.3162162162; 
	fragmentColor += texture2D(uTexture, vec2(oneStepRightvTextureCoord.x, oneStepRightvTextureCoord.y)) * 0.3162162162; 
	fragmentColor += texture2D(uTexture, vec2(twoStepsLeftvTextureCoord.x, twoStepsLeftvTextureCoord.y)) * 0.0702702703; 
    fragmentColor += texture2D(uTexture, vec2(twoStepsRightvTextureCoord.x, twoStepsRightvTextureCoord.y)) * 0.0702702703; 
